...
import { submitApplication } from "../../../services/applications/applicationService";
...
const SchemeDetails = () => {
    ...
    useEffect(() => {
        const checkIfSaved = async () => {
            try {
                ...
                const isSaved = savedSchemes.map(String).includes(String(id));
                setIsSaved(isSaved);
            } catch (error) {
    ...
    const handlePortalApply = async () => {
        ...
        try {
            setApplying(true);
            await submitApplication(scheme._id, applyNote);
                    ...
                    <header className="border-b border-gray-100 pb-6">
                        ...
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 rounded-xl p-4">
                            {scheme?.nodalMinistryName && (
                                <p className="text-gray-600">Ministry:{" "}
                                    <span className="font-medium text-gray-900">
                                        {typeof scheme.nodalMinistryName === "object"
                                            ? scheme.nodalMinistryName?.label
                                            : scheme.nodalMinistryName}
                                    </span>
                                </p>
                            )}
                    ...
                    <div className="py-4">
                        ...
                        {activeTab === 'apply' && (
                            <div className="space-y-4">
                                ...
                                <div className="mt-8 p-6 bg-white border border-[#74B83E]/40 rounded-xl shadow-sm">
                                    <h3 className="font-semibold text-gray-900 mb-2">Apply through this portal</h3>
                                    <p className="text-sm text-gray-600 mb-4">
                                        Submit your interest here for tracking. You must still complete any official steps
                                        (portal, CSC, or department) described above where applicable.
                                    </p>
                                    <textarea
                                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm min-h-[88px]"
                                        placeholder="Optional note (e.g. district, reference ID)"
                                        value={applyNote}
                                        onChange={(e) => setApplyNote(e.target.value)}
                                    />
                                    <button
                                        type="button"
                                        onClick={handlePortalApply}
                                        disabled={applying}
                                        className="mt-4 px-6 py-3 bg-[#74B83E] text-white rounded-lg font-medium hover:bg-[#629a33] disabled:opacity-50"
                                    >
