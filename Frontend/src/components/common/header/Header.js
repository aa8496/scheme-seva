...
import { Menu, X, Home, Info, FileText, LogIn, ShieldCheck, User, LayoutDashboard, ClipboardList } from 'lucide-react';
    ...
    return (
        <header className="bg-[#74B83E] h-20 flex items-center justify-between px-4 md:px-8 w-full relative">
            ...
            <nav className="hidden md:flex justify-between items-center gap-10">
                ...
                {isUserLoggedIn && (
                    <Link to="/my-applications" className="flex flex-col justify-center items-center text-white">
                        <ClipboardList className="w-7 h-7" />
                        <p className="font-semibold">My apps</p>
                    </Link>
                )}
                {isAdmin && (
                    <Link to="/admin" className="flex flex-col justify-center items-center text-white">
            ...
            {isOpen && (
                <div onClick={() => { setIsOpen(!isOpen); }} className="absolute top-20 left-0 right-0 bg-[#74B83E] md:hidden min-h-screen z-50 px-6">
                    ...
                    {isUserLoggedIn && (
                        <NavLink to="/my-applications" icon={<ClipboardList size={18} />}>My apps</NavLink>
                    )}
                    {isAdmin && (
                        <NavLink to="/admin" icon={<LayoutDashboard size={18} />}>Admin</NavLink>
                    )}
                </div>
            )}
        </header>
