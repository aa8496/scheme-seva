...
const withPublicVisibility = (filter = {}) => {
    const keys = Object.keys(filter).filter((k) => k !== "$and" && k !== "$or");
    const hasOther =
        keys.length > 0 ||
        (filter.$and && filter.$and.length) ||
        (filter.$or && filter.$or.length);
    if (!hasOther) return schemePublicFilter();
    return { $and: [schemePublicFilter(), filter] };
};
...
const getAllSchemes = async (req, res) => {
    try {
        ...
        const schemes = await Schemev2.paginate(withPublicVisibility({}), options);
        res.status(200).json({
...
const getSchemeById = async (req, res) => {
    try {
        ...
        if (!scheme || scheme.isActive === false) {
...
const getSchemeByCategory = async (req, res) => {
    try {
        ...
        const schemes = await Schemev2.paginate(
            withPublicVisibility({ schemeCategory: req.params.category }),
            options
        );
        res.status(200).json({
            schemes: schemes.docs,
...
const getFilteredSchemes = async (req, res) => {
    try {
        ...
        const schemes = await Schemev2.paginate(withPublicVisibility(filter), options);
        res.status(200).json({
            schemes: schemes.docs,
            totalPages: schemes.totalPages,
            currentPage: schemes.page,
...
const getFavoriteSchemes = async (req, res) => {
    try {
        ...
        res.status(200).json((user.favorites || []).map((fid) => String(fid)));
    } catch (error) {
        // Handle errors
